...
- **Efficient Spectral Estimation**: Estimate latent positions and
  community structures directly from ARD using `estimate_spectrum()`.
- **Sparse Graph Sampling**: Generate representative graphs from
  estimated models using `sample_graph()`, with support for large
  networks through memory-efficient chunked sampling.
- **Model-Based Simulation**: Generate synthetic ARD datasets using
  `simulate_ard_data()`. This function generates a network from a
  Degree-Corrected Stochastic Block Model (DCSBM) and simulates traits
  that are correlated with the underlying latent positions. ARD is then
  formed by aggregating these traits over the neighborhood of each node.
